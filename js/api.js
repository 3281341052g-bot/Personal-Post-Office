/* ═══════════════════════════════════════════════════
   API 适配层
   - 未配置服务器 → 使用 localStorage Mock 数据
   - 已配置服务器 → 调用 /api/*.php 真实接口
═══════════════════════════════════════════════════ */

(function () {
  'use strict';

  /* ── 判断是否在真实服务器上运行（非 file:// 协议） ── */
  ...
  async function request(url, options = {}) {
    const headers = { 'Content-Type': 'application/json', 'X-Session-Token': getToken() };
  ...
  window.api = {
    ...
    // ── 判断模式 ──
    isServerMode() { return IS_SERVER && !!getToken() && !getToken().startsWith('demo-'); },

    // ── 认证 ──
    getCurrentUser() { return JSON.parse(localStorage.getItem('ppo_user') || 'null'); },
    logout() { localStorage.removeItem('ppo_user'); localStorage.removeItem('ppo_token'); window.location.href = 'index.html'; },
    ...
    getMessage(id) {
      if (this.isServerMode() && id.startsWith('imap-')) {
    ...
    // ── 服务器配置 ──
    getServerConfig() {
      return JSON.parse(localStorage.getItem('ppo_server_config') || JSON.stringify({ host:'', smtpPort:465, imapPort:993, ssl:true, username:'', password:'', connected:false }));
    },
    saveServerConfig(cfg) {
      localStorage.setItem('ppo_server_config', JSON.stringify(cfg));
      if (IS_SERVER) {
        // 同步保存到服务器
        fetch(API_BASE + '/server-config.php', {
          method: 'POST',
    ...
    testConnection(cfg) {
      if (IS_SERVER) {
        return fetch(API_BASE + '/server-config.php?action=test', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-Session-Token': getToken() },
          body: JSON.stringify(cfg)
        }).then(r => r.json()).then(d => d.data || { success: false, message: d.error });
      }
      return Promise.resolve({ success: false, message: '本地演示模式，请部署到服务器后测试' });
    }
  };
